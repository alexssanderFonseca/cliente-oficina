...
Microserviço responsável pelo cadastro, busca, atualização e exclusão de clientes da oficina.

---

## 📋 Descrição

O **Serviço de Gerenciamento de Clientes** é um microserviço construído em **Java 21** com **Spring Boot** e **PostgreSQL**. A sua arquitetura é baseada no padrão **Ports and Adapters (Arquitetura Hexagonal)**, que isola a lógica de negócio de detalhes de infraestrutura.

O sistema foi projetado para simular processos típicos de uma oficina, como:

-   Gerenciamento de clientes (cadastro, busca, atualização, exclusão)
...
- [**Collection Insomnia - Cliente**](docs/collections/cliente-insomnia-collection.yaml) <!-- Placeholder: Update with actual path if exists -->
...
2.  Construa a imagem Docker do microserviço:
    ```bash
    docker build -t alexmarquesfa/clienteoficina:latest .
    ```
    (Substitua `alexmarquesfa/clienteoficina` pelo nome da sua imagem no Docker Hub, se aplicável).
3.  Aplique os manifestos Kubernetes usando Kustomize a partir da raiz do projeto:
    ```bash
    kubectl apply -k cliente/k8s/base/
    ```
    Ou, para um overlay específico (ex: ambiente de desenvolvimento):
    ```bash
    kubectl apply -k cliente/k8s/overlays/dev/ # ou producao, homolog, etc.
    ```

Após alguns instantes, todos os recursos (Pods, Services, Deployments, etc.) para o serviço de `cliente` estarão sendo criados.
...
```bash
minikube service cliente-app-service -n oficina-ns
```

#### 7. (Opcional) Monitoramento e Limpeza

- **Verificar status dos Pods:** `kubectl get pods -n oficina-ns -l app=cliente-app`
...
O projeto utiliza GitHub Actions para automação de integração e deploy contínuo. Os workflows específicos para este microserviço estão definidos em `.github/workflows` dentro do diretório `cliente/`.

### CI - Integração Contínua (`cliente/.github/workflows/ci-pipeline.yml`)

Este pipeline é acionado a cada `push` em uma branch com o padrão `feature/*` e quando há alterações no diretório `cliente/`. Seu objetivo é garantir a qualidade e a integridade do código antes que ele seja mesclado à `main`.
...
### CD - Deploy Contínuo (`cliente/.github/workflows/cd-pipeline.yml`)

Este pipeline é acionado automaticamente após um `merge` na branch `main` e quando há alterações no diretório `cliente/`. Seu objetivo é colocar a nova versão da aplicação em produção no ambiente da AWS.
